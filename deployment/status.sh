...
if docker ps --format "{{.Names}}" | grep -q "kali-workstation"; then
    ...
    docker inspect kali-workstation --format="  OT Network:   {{(index .NetworkSettings.Networks \"deployment_ot_network\").IPAddress}}" 2>/dev/null || echo "  OT Network:   Not connected"
    docker inspect kali-workstation --format="  SCADA Network: {{(index .NetworkSettings.Networks \"deployment_scada_network\").IPAddress}}" 2>/dev/null || echo "  SCADA Network: Not connected"
else
